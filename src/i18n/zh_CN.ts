/*
*******************************************************************
i18n/zh_CN.ts

Copyright (C) 2024 iEdon
Copyright (C) 2026 Luochancy

This file is part of a project derived from iedon-net-frontend.
Modified by Luochancy on 2026-06.

Licensed under the GNU General Public License v3.0.
See the LICENSE file in the project root for details.
*******************************************************************
*/
export default {
  header: {
    home: '首页',
    nodes: '节点互联',
    about: '关于',
    blog: '博客',
    lg: 'Looking Glass',
    health: '节点状态',
    manage: '管理',
    posts: '文章',
    signIn: '登录',
    signOut: '退出登录',
    signOutConfirm: '您确定要退出登录吗？',
  },
  common: {
    cancel: '取消',
    ok: '确定',
    yes: '是',
    no: '否',
    chartError: {
      loadFailed: '加载图表失败',
      refreshHint: '请刷新页面',
    },
  },
  notification: {
    error: '错误',
    info: '信息',
    warning: '警告',
    success: '成功',
    maintenance: '维护中',
    disconnected: '与服务器的连接已断开，正在重试...',
    reconnected: '已重新连接到服务器。'
  },
  packetHandler: {
    fetchException: '数据获取失败。',
    errMsg400_BAD_REQUEST: '您的请求无效。\n请检查输入格式后重试。',
    errMsg404: '很抱歉，当前请求的资源不可用。',
    errMsg405_METHOD_NOT_ALLOWED: '请求方法不被允许。',
    errMsg500_SERVER_ERROR: '很抱歉，服务器遇到问题。\n请重试或联系我们。',
    errMsg502_503: '很抱歉，服务不可用或正在维护。\n请重试或联系我们。',
    err_UNAUTHORIZED: '请登录以访问此资源。',
    err_ROUTER_NOT_AVAILABLE: '很抱歉，您请求的路由器不可用。\n请重试或联系我们。',
    err_ROUTER_OPERATION_FAILED: '很抱歉，配置会话时发生错误。\n请重试或联系我们。'
  },
  pages: {
    about: {
      title: '关于',
      subtitle: '了解本网络及相关服务。',
      comingSoon: 'Coming soon',
      underConstruction: 'This page is under construction.',
    },
    lg: {
      title: 'Looking Glass',
      subtitle: '查询 BGP 协议和路由表',
      protocols: '协议',
      routes: '路由',
      refresh: '刷新',
      name: '名称',
      protocol: '协议',
      state: '状态',
      since: '始于',
      info: '信息',
      noProtocols: '未找到协议',
      network: '网络',
      interface: '接口',
      type: '类型',
      from: '来源',
      metric: '度量值',
      primary: '首选',
      noRoutes: '此前缀未找到路由',
      prefixLabel: '前缀 (如 172.23.0.0/16 或 fd42::/16)',
      prefixPlaceholder: '输入 IP 前缀进行查询...',
      query: '查询',
      authRequired: '查询路由需要认证，请先登录。',
      enterPrefix: '在上方输入前缀以查询路由',
      protocolDetail: '协议详情',
      noDetail: '无详细信息',
      close: '关闭',
      loadProtocolsFailed: '加载协议失败',
      loadRoutesFailed: '加载路由失败',
      loadDetailFailed: '加载协议详情失败',
      searchPlaceholder: '按 ASN 搜索 (如 4242421857)',
      itemsPerPage: '每页条数:',
      enterPrefixWarning: '请输入要查询的前缀',
      authRequiredWarning: '需要认证',
      authRequiredToolsHint: '登录后可使用 Ping、Traceroute 和路由查询',
      selectNode: '选择节点',
      lgUnavailable: '该节点 Looking Glass 不可用',
      selectNodeFirst: '选择一个节点以查看其协议和路由',
      sessions: '个会话',
      back: '返回',
      rawData: '原始数据',
      view: '查看',
      traceSent: '发送',
      traceReceived: '接收',
      traceLoss: '丢失',
      traceAvgRtt: '平均 RTT',
      traceHop: '跳',
      traceRtt: 'RTT',
      enterTargetPrefix: '输入目标 IP 或前缀并点击查询',
      bgpNextHop: 'BGP 下一跳',
      routeChangeStats: '路由变更统计',
      detailImported: '导入',
      detailExported: '导出',
      detailPreferred: '首选',
      detailPreference: '优先级',
      detailImportLimit: '导入限制',
      detailTable: '表',
      detailInputFilter: '输入过滤',
      detailOutputFilter: '输出过滤',
      detailReceived: '接收',
      detailRejected: '拒绝',
      detailFiltered: '过滤',
      detailIgnored: '忽略',
      detailAccepted: '接受',
    },
    signIn: {
      step1: '查询',
      step2: '选择',
      step3: '挑战',
      step4: '欢迎',
      step1Introduction: '请确保您的对象已由 DN42 社区合并，并可从 WHOIS 数据库查询。',
      signInToPeerHub: 'Sign in to PeerHub',
      authenticateWithDn42: '使用 DN42 AS 验证身份',
      asnLogin: 'ASN 登录',
      weFoundMethods: '我们为您的 AS 找到了以下认证方式。',
      asnPlaceholder: '424242',
      pgpPlaceholder: '请输入签名',
      pgpPublicKeyPlaceholder: '-----BEGIN PGP PUBLIC KEY BLOCK-----',
      emailPlaceholder: '请输入验证码',
      emailSentInfo: '已发送邮件至 {email}，请检查收件箱或垃圾邮件。',
      emailSentSnackbar: '验证码已发送到 {email}',
      couldNotFindAuthMethodShort: '很抱歉，未能获取您的 AS 可用的认证方式。如有错误请联系我们。',
      copy: '复制',
      welcome: '欢迎',
      welcomeDesc: '您已成功登录 PeerHub',
      goToDashboard: '前往管理面板',
      asn: 'AS 号码',
      pleaseInput: '请输入或检查',
      signIn: '登录',
      couldNotFindAuthMethod: '很抱歉，未能获取您的 AS 可用的认证方式。\n可能原因：\n1. 您的对象未合并或同步，或发生错误。\n2. admin-c 或 mntner 中未找到有效条目。\n3. 认证方式不被本系统支持。\n如需帮助请联系我们，谢谢。',
      hi: '您好，',
      youCanAlso: '您还可以',
      authWithKioubit: '使用 Kioubit.dn42 认证',
      authWithOidc: '使用 auth.dn42 登录',
      signUp: '点击了解 DN42',
      queryingSignInMethod: '正在获取登录方式…',
      authenticateWith: '认证方式',
            noWebAuthMethods: '网页端暂不支持密码登录，请使用 PGP 或邮箱认证。',
      continue: '继续',
      step2Introduction: '我们为您的 AS 找到了以下认证方式。\n点击下方方式查看详情，选择一种继续。',
      errorOccurred: '很抱歉，发生错误。\n请重试或联系我们。',
      pleaseWait: '请稍候',
      step3Introduction: '请输入挑战文本。\n如使用邮箱认证，请查收您的邮箱（包括垃圾箱）。如未收到邮件可尝试其他登录方式。\n如使用 PGP 密钥，请粘贴完整公钥，明文签名挑战消息后粘贴签名输出及公钥（指纹需匹配）。',
      challengeHint: '邮箱或 PGP 签名',
      challengeText: '挑战文本',
      challengeTextPlaceholder: '邮箱验证码或 PEM 格式签名消息',
      pgpPublicKey: 'PGP 公钥',
      signInFailed: '登录失败',
      welcomeBack: '欢迎回来，{name}',
      step4Introduction: '您现在可以管理会话并配置新互联。',
      manageSessions: '管理互联会话',
      topPage: '首页',
      authMethods: [
        '使用密码',
        'PGP 明文签名',
        '邮箱接收挑战文本',
        'SSH 密钥签名'
      ],
      authMethodsTiny: [
        '密码认证',
        'PGP 认证',
        '邮箱认证',
        'SSH 认证'
      ],
      sshPlaceholder: '-----BEGIN SSH SIGNATURE-----',
      sshChallengePayload: '挑战承载',
      sshExampleCommand: '示例命令',
      sshSignHint: '将生成的 SSH 签名文件内容粘贴到下方输入框',
      sshPublicKey: 'SSH 公钥',
      sshPublicKeyHint: '您的公钥来自 DN42 注册表',
      sshVerifyFailed: 'SSH 签名验证失败，请检查私钥是否匹配 DN42 注册表中的公钥',
      changeAsn: '换个 ASN',
    },
    posts: {
      pleaseWait: '请稍候',
    },
    nodes: {
      search: '按名称或 IP 地址搜索…',
      nodes: 'BGP 网络节点',
      subTitle: '选择 BGP 路由器建立互联',
      copied: '详情已复制',
      copyRouterInfo: '复制路由器信息',
      connect: '连接',
      listView: '列表视图',
      gridView: '网格视图',
      pleaseSignIn: '请先登录。',
      statusOpen: '开放',
      statusOpenManuallyReview: '开放（人工审核）',
      statusClosed: '新用户关闭',
      statusFull: '已满员',
      statusCapacity: '容量',
      routerOffline: '路由器离线',
      routerOfflineDescription: '该路由器似乎已离线（超过 15 分钟未更新）。请稍后重试或联系管理员。',
      maintenanceMode: '维护模式',
      systemMetrics: '系统指标',
      uptime: '在线时长',
      loadAvg: '负载均值',
      txRx: '收发流量',
      tcpUdp: 'TCP / UDP',
      router: '路由器',
      kernel: '内核',
      agent: 'API',
      totalRouters: '节点总数',
      totalSessions: '已配置会话',
      availableForAuto: '即时互联',
      // Empty states
      noRoutersMatch: '没有路由器匹配您的搜索',
      noRoutersAvailable: '暂无可用路由器',
      // Regions
      regions: {
        'Europe': '欧洲',
        'North America': '北美洲',
        'South America': '南美洲',
        'Africa': '非洲',
        'Asia-S': '南亚',
        'Asia-SE': '东南亚',
        'Asia-E': '东亚',
        'Pacific&Oceania': '太平洋和大洋洲',
        'Antarctica': '南极洲',
        'Asia-N': '北亚',
        'Asia-W': '西亚',
        'Central Asia': '中亚',
        'Other Region': '其他地区',
        'All': '全部'
      }
    },
        health: {
      title: '节点状态',
      subtitle: '所有边缘节点的实时服务状态',
      all: '全部',
      online: '在线',
      offline: '离线',
      viewingSpecificNode: '正在查看特定节点',
      noNodesAvailable: '暂无可用节点',
      noOnlineNodes: '无在线节点',
      noOfflineNodes: '无离线节点',
      statusOnline: '在线',
      statusOffline: '离线',
      heartbeat: '心跳',
      load: '负载',
      network: '网络',
      hideDiagnostics: '隐藏诊断',
      diagnostics: '诊断',
      kernel: '内核',
      bird: 'BIRD',
      agent: 'Agent',
      tcpUdp: 'TCP / UDP',
      version: '版本',
      signInToView: '登录后查看节点诊断信息',
      adminRequired: '需要管理员权限才能查看节点诊断',
      loadFailed: '加载节点状态数据失败',
      pageTitle: '节点状态',
      na: 'N/A',
      timeAgo: {
        seconds: '秒前',
        minutes: '分钟前',
        hours: '小时前',
        days: '天前',
      },
    },
peering: {
      couldNotGetData: '无法获取节点数据。',
      step1: '功能',
      step2: '接口',
      step3: '设置',
      step4: '完成',
      linkType: '链路类型',
      interface: '接口',
      bgpExtensions: 'BGP 扩展',
      routingPolicy: '路由策略（您的角色）',
      routingPolicyTypes: {
        FULL: '全表 / 全中继',
        FULL_DESC: '⭐ DN42 默认推荐。收发所有有效路由。',
        TRANSIT: '中继',
        TRANSIT_DESC: '传送本地及直接下游路由，接收所有有效路由。',
        PEER: '对等',
        PEER_DESC: '传送本地及直接下游路由，接收您的路由（AS Path = 1）。',
        DOWNSTREAM: '下游',
        DOWNSTREAM_DESC: '传送所有有效路由，接收您的路由（AS Path = 1）。',
        UPSTREAM: '上游（管理员）',
        UPSTREAM_DESC: '接收所有有效路由，向远端传送本地及直接下游路由。'
      },
      v4v6force: '💡 在端点前加 “v4.” 或 “v6.” 前缀可强制使用 IPv4 或 IPv6（如支持）。',
      openvpn: 'OpenVPN',
      wireguard: 'WireGuard',
      ipsec: 'IPSec',
      gre: 'GRE（IPv4）',
      ip6gre: 'GRE（IPv6）',
      direct: '直连以太网',
      'mp-bgp': '多协议 BGP',
      'extended-nexthop': '扩展 Nexthop',
      step1Introduction: '此节点支持以下功能。\n可多选。路由策略仅供娱乐，建议使用 BGP 社区（65535, 65281）和（65535, 65282）配合“全表”精确控制路由传播。',
      step2Introduction: '以下设置有助于配置您的接口',
      useIpv4: '启用 IPv4',
      useIpv6: '使用 IPv6 ULA',
      useIpv6LinkLocal: '使用并优先 IPv6 Link Local',
      endpoint: '端点',
      credential: '公钥',
      mtu: '接口 MTU',
      mustEnableAtleastOneProtocol: '至少需启用一种 IP 协议。',
      mustEnterTunnelInformation: '需填写隧道信息，包括端点和凭证。',
      inputValid: '信息无效，请检查后重试。',
      inputValidMTU: '请输入有效 MTU（1280-9999）。',
      back: '返回修改',
      backTop: '返回',
      step3Introduction: '您的互联申请详情，请核对。',
      ipv4: 'DN42 IPv4',
      ipv6: 'DN42 IPv6',
      asn: '对端 ASN',
      ipv6LinkLocal: 'IPv6 Link Local',
      tunnelEndpointHint: '（可选）隧道端点格式：主机名或 IP:端口',
      tunnelCredentialHint: '您的隧道凭证或公钥。',
      directInfo1: '无加密直连以太网对等',
      directInfo2: '必须在两侧预配置接口',
      directInfo3: '需要远端对等方的IP',
      directSessionLimit: '同一端点在同一节点上只能创建 1 个会话',
      directLanInfo: '您的对等 IP 地址必须在以下子网内',
      directTitle: '直连以太网',
      directLocalInterface: '本端接口',
      thankYou: '感谢',
      step4Introduction: '正在配置您的会话，请稍后查看互联状态。',
      step4IntroductionReview: '此会话需人工审核，我们将尽快处理您的请求。',
            adminPeering: '管理员互联',
      stepPrefix: '步骤',

      reuseExistingConfig: '沿用我之前配置的端点或端口（我们的）',
    },
    landing: {
      startPeering: '即时互联',
      grafanaOverview: '在 Grafana 查看概览',
      globalNetworkMap: '全球网络地图',
      mapDescription: '探索我们的 DN42 节点和互联会话全球网络。',
      whyChoose: '为什么选择我们的网络？',
      automaticPeering: '自动互联',
      automaticPeeringDescription: '自动配置互联会话，无需人工干预。\n系统为您处理技术细节。',
      globalNetwork: '全球网络',
      globalNetworkDescription: '全球多地接入，优化路由和冗余，畅游 DN42 网络。',
      secureAndReliable: '安全可靠',
      secureAndReliableDescription: '行业标准加密（WireGuard 等），高可用性，42% 在线保证。^_^ 乐趣如 DN42 本身。',
    },
    manage: {
      closeMenu: '关闭菜单',
      openMenu: '打开菜单',
      mySessions: '我的会话',
      myAccount: '我的账户',
      manageSessions: '管理会话',
      managePosts: '管理文章',
      manageConfig: '站点设置',
      manageNodes: '管理节点',
      settings: '设置',
      allSessions: '全部会话',
      session: {
        search: '按 ASN 或 IP 地址搜索…',
        node: '节点',
        asn: 'ASN',
        ipAddresses: 'IP 地址',
        type: '接口类型',
        status: '状态',
        action: '操作',
        info: '信息',
        viewMetrics: '查看指标',
        newPeeringSession: '新建互联会话',
        probeStatus: '连通状态',
        statusCode: {
          '0': '已删除',
          '1': '已禁用',
          '2': '已启用',
          '3': '待审批',
          '4': '排队设置',
          '5': '排队删除',
          '6': '检测到问题',
          '7': '已拆除',
        },
        enable: '启用',
        disable: '禁用',
        remove: '移除',
        approve: '批准',
        edit: '编辑',
        areYouSure: '确定吗？',
        ok: '确定',
    chartError: {
      loadFailed: '加载图表失败',
      refreshHint: '请刷新页面',
    },
      },
      account: {
        asn: 'ASN',
        asName: 'AS 名称',
        descr: '描述',
        country: '国家',
        org: '组织',
        status: '状态',
        adminC: 'Admin-C',
        techC: 'Tech-C',
        mntBy: 'Mnt-By',
        mntLower: 'Mnt-Lower',
        mntRoutes: 'Mnt-Routes',
        notify: '通知',
        source: '来源',
        remarks: '备注',
        whoisUnavailable: 'WHOIS 数据暂不可用。',
      },
      config: {
        changeConfig: '修改站点配置',
        save: '保存',
        netAsn: '管理员 ASN',
        netName: '网络名称',
        netDesc: '网络描述',
        footerText: '页脚文本',
        maintenanceText: '维护文本'
      },
      posts: {
        title: '标题',
        category: '分类',
        updatedAt: '更新时间',
        createdAt: '创建时间',
        addOrEdit: '添加或编辑文章',
        add: '添加文章',
        view: '查看',
        edit: '编辑',
        content: '内容',
        close: '关闭'
      },
      nodes: {
        sessionCapacity: '会话容量',
        openPeering: '开放互联',
        autoPeering: '自动互联',
        public: '公开（可见性）',
        location: '位置',
        description: '描述',
        name: '名称',
        addOrEdit: '添加或编辑节点', add: '添加节点',
        agentSecret: '代理密钥',
        callbackUrl: '回调 URL',
        allowedPolicies: '可用路由策略',
        sessionCount: '会话',
        agentStatus: 'Agent 状态',
        status: '状态',
        linkTypes: '链路类型',
        rotateSecret: '重置密钥',
        resetSecretHint: '留空则保持当前密钥不变',
        secretSet: '已配置',
        secretNotSet: '未配置',
        subtitle: '管理节点、Agent 和 Peering 配置',
        view: '查看',
        // Detail page
        nodeNotFound: '节点未找到',
        backToNodes: '返回节点列表',
        basicConfiguration: '基础配置',
        publicLabel: '公开',
        openPeeringLabel: '开放互联',
        autoPeeringLabel: '自动互联',
        peeringCapabilities: 'Peering 能力',
        sessionCapacityLabel: '会话容量',
        callbackUrlLabel: '回调 URL',
        linkTypesLabel: '链路类型',
        bgpExtensions: 'BGP 扩展',
        allowedPoliciesLabel: '可用策略',
        agentStatusLabel: 'Agent 状态',
        statusLabel: '状态',
        uptime: '运行时间',
        loadAverage: '负载均值',
        bandwidth: '带宽',
        agentVersion: 'Agent 版本',
        kernel: '内核',
        agentNotReporting: 'Agent 未上报 — 无指标数据',
        forceSync: '强制同步',
        diagnostics: '诊断',
        diagnosticsComingSoon: 'LG 工具、回调测试和探针诊断即将推出',
        dangerZone: '危险操作',
        deleteNode: '删除节点',
        deleteNodeConfirm: '删除节点？',
        deleteNodeDesc: '将永久删除',
        deleteNodeWarning: '，此操作不可撤销。',
      }
    },
    metrics: {
      sessionMetrics: '会话指标',
      viewInGrafana: '在 Grafana 中查看',
      lastError: '最新错误',
      noErrors: '无错误',
      probeStatus: {
        title: '连通性诊断',
        hint: '来自路由器探针的 IPv4/IPv6 自动连通性检测。',
        labels: {
          testedOk: '测试通过',
          noRouting: '无路由',
          nat: '检测到 NAT',
          notAvailable: '暂无'
        },
        descriptions: {
          testedOk: '所有连通性检测均已成功通过。',
          noRouting: '对等互联的 IP 传输配置已损坏。某些对等方可能不希望提供 IP 传输，在这种情况下可以安全地忽略此状态。',
          nat: '检测到配置错误。传输数据包正在使用网络地址转换。',
          notAvailable: '状态信息不可用。'
        }
      },
      back: '返回',
      clickToCopy: '点击复制',
      refresh: '刷新',
      sessionId: '会话 ID',
      interaceType: '接口类型',
      interfaceName: '接口名称',
      bgpExtensions: 'BGP 扩展',
      routingPolicy: '路由策略',
      mtu: 'MTU',
      sessionInfo: '会话信息',
      lastUpdated: '指标更新时间',
      createdAt: '创建时间',
      updatedAt: '更新时间',
      loadingMetrics: '正在加载指标…',
      noMetricsFound: '未找到指标',
      noMetricsFoundDesc: '无法找到此会话的指标数据。',
      goBack: '返回',
      fetchError: '获取指标数据失败',
      invalidSession: '会话参数无效',
      noData: '暂无数据',
      notAvailable: 'N/A',
      loading: '加载中…',
      none: '无',
      unknown: '未知',
      sessionDetails: '会话详情',
      nextUpdate: '下次更新',
      networkInfo: '网络信息',
      ipv4Address: 'IPv4 地址',
      ipv6Address: 'IPv6 地址',
      ipv6LinkLocal: 'IPv6 Link Local',
      bgpMetrics: 'BGP 指标',
      received: '导入',
      advertised: '导出',
      bgpDetails: 'BGP 详情',
      bgpSession: 'BGP 会话',
      routesReceivedIPv4: 'IPv4 导入',
      routesAdvertisedIPv4: 'IPv4 导出',
      routesReceivedIPv6: 'IPv6 导入',
      routesAdvertisedIPv6: 'IPv6 导出',
      interfaceMetrics: '接口指标',
      interfaceDetails: '接口详情',
      rxBytes: '接收字节',
      txBytes: '发送字节',
      bytes: '字节',
      txCurrent: '当前发送',
      rxCurrent: '当前接收',
      bgpStatus: {
        Close: '已停止',
        Established: '已建立',
        Idle: '空闲/接口 IP 错误',
        Active: '连接失败',
        Connect: '连接中',
        Open: '协商中',
        OpenSent: '协商中',
        OpenConfirm: '确认中',
        Error: '错误',
        Unknown: '未知',
        No: '无数据',
      },
      networkMetrics: '网络指标',
      rtt: 'RTT',
      currentRtt: '当前 RTT',
      packetLoss: '丢包率',
      timeout: '超时',
      interfaceIPv4: 'IPv4',
      interfaceIPv6: 'IPv6',
      interfaceIPv6LinkLocal: 'IPv6 Link Local',
      interfaceMAC: 'MAC 地址',
      interfaceMTU: 'MTU',
      interfaceStatus: '状态',
      currentRates: '当前收发',
      txTotal: '总发送',
      rxTotal: '总接收',
      routes: '路由',
      trafficTotal: '累计流量',
      bgpPeerName: '对端名称',
      bgpState: '状态',
      bgpSince: '自',
      sessionStatus: '会话状态',
      detailedMetrics: '详细指标',
      clickToViewDetails: '点击查看详细信息',
      clickToViewChart: '点击查看图表',
      timeAgo: {
        seconds: '秒前',
        minute: '分钟前',
        minutes: '分钟前',
        hour: '小时前',
        hours: '小时前',
        day: '天前',
        days: '天前'
      }
    }
  }
}
